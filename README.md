...
- **Family/communal framing's association survives - but only once the right quantity is tested, and only in two pooled region categories.** The interaction coefficients this analysis first reported test whether a region's slope differs from the Africa baseline; they do *not* test whether family framing does anything *within* a region. Getting the within-region quantity right takes one further step, because family framing is interacted with three moderators (period, region, loan size): its main effect is the slope only at *their* reference levels too, so "main effect + region term" is still a slope at one unrepresentative cell. The correct quantity is the **average within-region slope** - each region's slope averaged over its own mix of periods and loan sizes, computed as a weighted linear contrast in `notebooks/2_full_dataset_modeling.ipynb` §7.2. Under clustering, only **Middle East (-0.124) and Central America (-0.062)** clear p < 0.05, both p < 0.0001, both in the faster-funding direction (duration model: negative = faster). This is the strongest narrative-framing evidence in the project - with one estimand caveat stated plainly: each is an average over a *pooled category* (Middle East = Palestine + Yemen; Central America = Honduras + Nicaragua). The model never estimates a separate slope for any individual country, so a pooled result could be driven mostly by one constituent country; locating any country-level effect would take a country-stratified design.
- **But those two categories are two countries each, so this is a pooled-category result on a thin cluster base - not a regional finding, and not a per-country one either** - "Middle East" here is Palestine and Yemen (14,946 loans); "Central America" is Honduras and Nicaragua (59,391 loans); "North America" is Haiti alone. Because clustering by country is exactly what stops same-country loans counting as independent evidence, a two-cluster group carries very little of it. Mechanically the clustered covariance is still estimated from all 48 country clusters, so the standard error is not computed from two clusters alone - but the region-specific contrast is *identified* only by those two, so it inherits whatever is idiosyncratic about them and cannot separate a family-framing association from a Palestine/Yemen or Honduras/Nicaragua country effect. Treat it as exploratory and worth a country-stratified test (one designed to show which constituent countries, if any, drive the pooled association), not as a region-level or country-level finding to act on.
