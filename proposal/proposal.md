...
- Does that association differ across pre-specified segments — analysis period, region group, and loan-size band by default; sector as an explicitly scoped, adequately-represented-only extension; gender classification and group status are modeled as structural controls now, with segment interactions for them a natural next pre-specified extension?
...
A robust OLS model regresses log(1 + funding-speed-days) on pre-specified narrative, structural, and contextual predictors, with **heteroskedasticity-robust (HC3) standard errors**; a parallel binomial GLM models funding within 24 hours on the log-odds scale. Both report every pre-specified coefficient with a 95% confidence interval — never selected post hoc by significance — using association language throughout. Both also degrade gracefully rather than misleadingly: if a model cannot be reliably identified on a given sample, it reports a labeled diagnostic instead of an unstable coefficient, while any other well-identified model in the same run is still reported in full. This is not hypothetical: on our development sample the 24-hour binary model hits exactly this failure mode (**quasi-complete separation** at only 100 rows) and correctly reports a diagnostic, while the duration model fits without issue.

Three theoretically distinct framing measures anchor the narrative predictors, following established crowdfunding-narrative research (Allison et al., 2015; Moss et al., 2015) and normalized per 100 words for comparability across description lengths:

- **Communal/family** framing
- **Agentic/competence** framing
- **Urgency** appeals

The default model fits three **pre-specified interactions** — family framing × analysis period, × region group, and × loan-size band — each independently dropped if a sample cannot support it; all three survive on the development sample. Region group uses a fixed observation-count threshold, not a hardcoded region list, so it automatically re-derives which regions are adequately represented from whichever data is actually passed in — the same principle sector needs before it can move from its current opt-in status to default. On this sample, neither loan amount nor sector reaches conventional significance (n=100, illustrative of feasibility only) — several sector estimates carry wide intervals at this size, and any predictor lacking variation is automatically dropped.

### Predictive Benchmarks

A gradient-boosted regressor (`HistGradientBoostingRegressor`) benchmarks nonlinear predictive performance against a training-median baseline and a regularized linear model, using **permutation importance** on held-out data, scored in day-space so rankings match the units a reader interprets. A parallel leakage-safe chronological classifier (`HistGradientBoostingClassifier`) predicts funding within 24 hours, reporting **ROC AUC, PR AUC, and Brier score** on the untouched holdout — fulfilling the binary outcome's own predictive-validation requirement, distinct from the explanatory GLM above. On the development sample it discriminates well (holdout ROC AUC 0.88, PR AUC 0.79) despite a 20-row holdout.
...
The duration model estimates average log-transformed pace; the 24-hour outcomes (explanatory and predictive) isolate what predicts the fastest-funding loans. Chronological validation yields an honest transfer estimate, and the nonlinear benchmark's feature ranking offers a complementary, held-out check on which levers the coefficients single out.

For Kiva field partners, the managerial deliverable is a **segment-by-framing opportunity matrix** (which narrative choice is worth the writing effort, for which segment, flagged by confidence — not asserted where the interval is wide) plus prioritized copy guidance field partners can pilot and a field-test agenda for validating the highest-confidence associations. Because recommendations are **associations with confidence intervals, not causal claims**, they are testable hypotheses to validate, not settled prescriptions. Guidance should improve clarity, specificity, and relevant detail — and must **not** coach exaggerated hardship, manufacture unreal urgency, or suppress relevant borrower information; the same funding-speed gain pursued through manipulation would undermine the trust prosocial lending depends on. Every limitation — sample size, the funded-only outcome boundary, the associational nature of every estimate — is reported alongside the findings.

**One-week execution sequence:** (1) full-dataset schema/coverage audit; (2) freeze segment-grouping thresholds against the audited counts; (3) feature-extraction QA; (4) re-run the chronological, explanatory, and classifier fits; (5) diagnostics and sensitivity checks; (6) build the segment-by-framing opportunity matrix; (7) reproducibility check and final write-up. The pipeline already runs this sequence end to end on the development sample, so the full-data pass is a re-run, not new code; the region/loan-size thresholds and pruning logic already degrade gracefully if the full data's categories or date range differ from this sample's.
